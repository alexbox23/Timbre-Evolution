...
def load_wav(path, sr):
    ret = librosa.core.load(path, mono=False, sr=sr)[0]
    if len(ret.shape) >= 2:
        ret = ret[0]
    return ret
...
def save_wav(wav, path):
    wav *= hparams.byte_norm / max(0.01, np.max(np.abs(wav)))
    librosa.output.write_wav(path, wav, hparams.sample_rate, True)
